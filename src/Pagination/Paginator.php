...
final class Paginator
{
    ...
    public function __construct(
        ...
        if ($pageSize < 1) {
            throw new \InvalidArgumentException(\sprintf('The page size must be 1 or more, "%d" given.', $pageSize));
        }
    }
    ...
    public function paginate(int $page = 1): self
    {
        ...
        $paginator = new OffsetPaginator(fetchJoinCollection: true, useOutputWalkers: $useOutputWalkers);

        // the window carries the offset and the page size explicitly, so the query
        // itself is left untouched and the paginator can be reused for any page
        $window = Window::fromPageNumberAndSize($this->currentPage, $this->pageSize);
        $windowPage = $paginator->paginate($query, $window);
